...
export class AuthService {
  ...
  public async generateAccessToken(payload: {
    ...
    const Admin = await this.admin
      .findById(payload.id)
      .populate({ path: 'role', select: 'modules title' })
      .select('role phone_number org')
      .exec()
    ...
    return jwt.sign(
      { id: Admin['_id'], modules: Admin.role.modules, org: Admin.org },
      'secret_key',
      {
        expiresIn: '1m'
      }
  ...
  public async login(payload: { phoneNumber: string; password: string }) {
    const Admin = await this.admin
      .findOne({ phone_number: payload.phoneNumber })
      .select('phone_number org password')
      .populate('role', 'title modules')
      .exec()
    ...
    const refresh_token = jwt.sign(
      {
        data: { userId: Admin['_id'], orgId: Admin.org },
